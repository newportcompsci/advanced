...
title: "Lesson 1.04 - Complexity Continued"
slides: false
---
...
> In general an algorithm is O(log(n)) if it takes constant time to reduce the 
> problem by a fractional amount (typically 1/2). If it takes constant time 
> to reduce the problem by a constant amount then the problem is merely O(n).
